...
var GlobeLayerKapsule = Kapsule({
  ...
  stateInit: function stateInit() {
    // create globe
    var globeGeometry = new THREE$g.SphereGeometry(GLOBE_RADIUS, 75 /* default value first */, 75 /* default value first */);
  ...
  update: function update(state, changedProps) {
    ...
    if (changedProps.hasOwnProperty('widthSegments')) {
      if (typeof Number(state.widthSegments) != "number") {
        console.log('width segments is not a valid number');
      } else {
        let globe = threeJSScene.getObjectByName('globe');
        let hexColor = globe.material.color.getHex();
        let globeColor = new THREE$g.MeshPhongMaterial({ color: hexColor });
        let globeGeometry = new THREE$g.SphereGeometry(GLOBE_RADIUS, state.widthSegments, state.heightSegments);
        let globeObj = new THREE$g.Mesh(globeGeometry, globeColor);
  
        threeJSScene.remove(globe);
        globeObj.rotation.y = -Math.PI / 2; // face prime meridian along Z axis
        globeObj.__globeObjType = 'globe'; // Add object type
        globeObj.name = 'globe';
        globeObj.material.color.setHex(hexColor)
        threeJSScene.add(globeObj);
      }
    }

    if (changedProps.hasOwnProperty('heightSegments')) {
      if (typeof Number(state.heightSegments) != "number") {
        console.log('height segments is not a valid number');
      } else {
        let globe = threeJSScene.getObjectByName('globe');
        let hexColor = globe.material.color.getHex();
        let globeColor = new THREE$g.MeshPhongMaterial({ color: hexColor });
...
var linkedGlobeLayerProps = Object.assign.apply(Object, _toConsumableArray(['globeImageUrl', 'bumpImageUrl', 'showGlobe', 'showGraticules', 'showAtmosphere', 'atmosphereColor', 'atmosphereAltitude', 'widthSegments', 'heightSegments'].map(function (p) {
